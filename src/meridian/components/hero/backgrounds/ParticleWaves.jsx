...
let themeStart = [0.14, 0.72, 1.0];
let themeEnd = [1.0, 0.32, 0.95];
let currentAccent = '';
let frameCount = 0;

const fpsMeter = { start: () => {}, tick: () => {}, stop: () => {} };
...
function handleResize() {
  if (!holderRef.current || !canvasRef.current || !gl) return;
  ...
  if (
    ...
    canvasRef.current.width = canvasWidth;
    canvasRef.current.height = canvasHeight;
    canvasRef.current.style.width = `${holderWidth}px`;
    canvasRef.current.style.height = `${holderHeight}px`;
...
`;
  ...
  if (typeof document !== 'undefined') {
    const updateColors = () => {
      ...
      const accent = style.getPropertyValue('--meridian-accent').trim();
      const bgLight = style.getPropertyValue('--meridian-hero-bg-light').trim();
      if (accent) {
        themeStart = hexToRgb(accent) || [0.14, 0.72, 1.0];
        // Generate a contrasting end color to ensure a nice gradient, 
        // since some bgLight values like #fafafa are too dull for particles.
        // Let's use the accent color and shift its hue slightly for the end color
        if (bgLight.toLowerCase() === '#fafafa' || bgLight.toLowerCase() === '#5a6fd4' || bgLight.toLowerCase() === '#f2f2f2') {
           // We create a slightly offset version of the accent
           themeEnd = [Math.max(0, themeStart[0] - 0.2), Math.min(1, themeStart[1] + 0.2), themeStart[2]];
        } else {
           themeEnd = hexToRgb(bgLight) || [1.0, 0.32, 0.95];
        }
        rebuildGrid({ onlyColors: true });
      }
    };
    ...
    themeObserver = new MutationObserver((mutations) => {
      for (const m of mutations) {
        if (m.attributeName === 'data-meridian-palette' || m.attributeName === 'style') {
          updateColors();
        }
      }
    });
    themeObserver.observe(document.documentElement, { attributes: true, attributeFilter: ['data-meridian-palette', 'style'] });
  }
  ...
  if (resizeObserver && holderRef.current) {
  ...
  return (
    <div ref={holderRef} className="absolute inset-0 pointer-events-none" style={{ zIndex: 0 }}>
